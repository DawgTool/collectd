...
I<TCP> over the "http" or "line based" protocol with a default port 4242.
The data will be sent in blocks of at most 1428 bytes to minimize the number
of network packets.
...
=item B<URL> I<URI Address>

Hostname or address to connect to. Defaults to C<http://localhost:4242/api/put>.
Acceptable protocols = [http, raw]

=item B<DataFormat> I<TAGS>|<GRAPHITE>

Sets the data format which is sent to TSDB. Defaults to C<TAGS>.
EXAMPLE (GRAPHITE):
{
...
=item B<SendFormat> I<HTTP>|<PUT>

Sets the send format in which to post data to TSDB. Defaults to C<HTTP>.
HTTP uses the standard curl http POST syntax, while PUT will send a raw
'put' record to the TSDB server.
...
=item B<Tags> I<key/value>

Combination for I<key/value> pairs that will be appended to the tags generated
by the metric. Please note that packaged versions of openTSDB limit the number
of tags to B<8>. More the 8 tags will cause the record to be B<rejected>. If
compiled from source, this value can be set higher. B<32> tags is the current
limit for this plugin.
...
If set the B<true>, append the type and name of the I<Data Source> (DS) to
the "metric" identifier. If set to B<false> (the default), this is only done
when there is more than one DS. DS is type of 'value' is always ignored.

=item B<ReplaceChar> B<char>

Defines the single character replacement which is used to cleanse names
and values before they are sent to TSDB. Any character matching the following
will be replaced with this character. Default is C<_>.
FORBIDDEN " \t\"\\:!/()\n\r"
